...
# test dataset dir
DATASET_DIR = os.path.join(TEST_DATA_DIR, "dataset_ner")
...
layoutlm_args = {'output_dir': CHECKPOINT_DIR,
            'overwrite_output_dir': True,
            'do_train': True,
            'do_eval': True,
            'do_predict': True,
...
def test_training(args, teardown_cleaning):
    ...
    # test predictions
    # first make sure we re-import the ScoringService for tyhe first time
    ...
    assert all(len(p) == len(text.split()) for p, text in zip(result["pred"], input["text"]))
    assert not ("proba" in result)